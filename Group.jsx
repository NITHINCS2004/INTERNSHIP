...
const Group = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (action === 'create') {
            ...
            updatedGroups[groupIndex].members.push(email);
            sessionStorage.setItem("groups", JSON.stringify(updatedGroups));
            setGroups(updatedGroups);
            alert(`Successfully joined group: ${selectedGroup}`);
        }
        ...
        <div>
            ...
            {action && action !== 'invite' && (
                <form onSubmit={handleSubmit}>
                    {action === 'create' ? (
                        <>
                            <input type="text" value={groupname} onChange={(e) => setGroupname(e.target.value)} required placeholder="Group Name" />
            ...
            {action === 'invite' && (
                <div>
                    <input type="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} required placeholder="Enter Email" />
                    <select onChange={(e) => setSelectedGroup(e.target.value)} required>
                        <option value="">Select a group</option>
                        {groups.map((group, index) => (
                            <option key={index} value={group.groupname}>{group.groupname}</option>
